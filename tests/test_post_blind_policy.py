"""DEV POLICY CLOSURE — 4 policy phát sinh sau blind runtime test (Vi Ngọc Phương).

Không đụng `input/Vi Ngọc Phương/*.pdf`. Dùng PDF/JSON tổng hợp từ
...
def test_golden_van_pass_provider_agent(golden_root: Path):
    reports = run_all_golden(golden_root, provider_name="agent", provider_config={"analysis_root": str(golden_root / "analysis")})
    assert reports
...
def test_blind_analysis_json_hash_khong_doi(repo_root: Path):
    folder = repo_root / "analysis" / "Vi Ngọc Phương"
    if not folder.is_dir():
        pytest.skip("Không có analysis/Vi Ngọc Phương trong workspace này.")
...
def test_vi_phuong_pdf_hash_khong_doi(repo_root: Path):
    folder = repo_root / "input" / "Vi Ngọc Phương"
    if not folder.is_dir():
        pytest.skip("Không có input/Vi Ngọc Phương trong workspace này.")
