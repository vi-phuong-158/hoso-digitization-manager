...
def test_agent_gom_trang_khac_segmenter_thi_sang_review(
    tmp_path: Path, hai_folder: Path, hai_analysis_root: Path, catalog
):
    """Agent tách bìa ra khỏi văn bằng -> lệch segmenter local -> REVIEW."""
    src = hai_analysis_root / "Synthetic Person" / "Bang cap cua HAI.json"
    ...
    for name in ("Quyet dinh dieu dong HAI", "Phieu bo sung hs dang vien 2020 HAI"):
        (root / f"{name}.json").write_text(
            (hai_analysis_root / "Synthetic Person" / f"{name}.json").read_text(encoding="utf-8"),
            encoding="utf-8",
        )
...
def test_provider_khong_bao_gio_tra_ve_ten_file(catalog, hai_folder: Path, hai_analysis_root: Path):
    p = AgentAnalysisProvider({"catalog": catalog, "analysis_root": hai_analysis_root})
