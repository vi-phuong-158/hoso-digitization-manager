...
def test_golden_acceptance(golden_path: Path, provider: str, golden_root: Path):
    ...
    config = {"analysis_root": str(golden_root / "analysis")} if provider == "agent" else None
    report = run_golden_file(golden_path, root=golden_root, provider_name=provider, provider_config=config)
    assert report.passed, f"provider={provider}\n" + "\n".join(str(f) for f in report.failures)


def test_agent_va_fixture_cho_cung_ket_qua_segmentation(golden_root: Path):
    a = run_golden_file(
        list_golden_files(golden_root)[0],
        root=golden_root,
        provider_name="agent",
