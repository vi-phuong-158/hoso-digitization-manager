...
def test_co_it_nhat_mot_file_golden(repo_root: Path):
    assert list_golden_files(repo_root), "Thiếu bộ golden trong test_cases/"
...
def test_golden_acceptance(golden_path: Path, provider: str, repo_root: Path):
    """Golden phải xanh với CẢ fixture (test) lẫn agent (runtime Antigravity thật)."""
    report = run_golden_file(golden_path, root=repo_root, provider_name=provider)
    assert report.passed, f"provider={provider}\n" + "\n".join(str(f) for f in report.failures)


def test_agent_va_fixture_cho_cung_ket_qua_segmentation(repo_root: Path):
    a = run_golden_file(list_golden_files(repo_root)[0], root=repo_root, provider_name="agent")
    f = run_golden_file(list_golden_files(repo_root)[0], root=repo_root, provider_name="fixture")
...
def test_golden_bao_phu_du_moi_case(repo_root: Path):
    reports = run_all_golden(repo_root)
    total_expected = 0
    for path in list_golden_files(repo_root):
...
def test_golden_khong_bo_sot_trang_nao(repo_root: Path):
    reports = run_all_golden(repo_root)
    for r in reports:
...
def test_golden_khong_ghi_gi_ra_output_hay_review(repo_root: Path):
    before_out = _snapshot(repo_root / "output")
    before_rev = _snapshot(repo_root / "review")
    run_all_golden(repo_root)
    assert _snapshot(repo_root / "output") == before_out
    assert _snapshot(repo_root / "review") == before_rev


def test_golden_khong_sua_file_nguon(repo_root: Path):
    before = _snapshot(repo_root / "input")
    run_all_golden(repo_root)
    assert _snapshot(repo_root / "input") == before
